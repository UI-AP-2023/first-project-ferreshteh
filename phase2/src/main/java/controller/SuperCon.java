    ...
    public void productManagement(String function) {
        ...
        if (strings[0].equalsIgnoreCase("add")) {
            if (strings[1].equalsIgnoreCase("machine")) {
                ...
                LocalDate dateExpiration = LocalDate.parse(strings[5], formatter);
                LocalDate dataProduction = LocalDate.parse(strings[6], formatter);

                addMeal(strings[2], strings[3], "meal", Integer.parseInt(strings[4]), dataProduction, dateExpiration);
